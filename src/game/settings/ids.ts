/**
 * Represents all player sprite positions.
 */
export enum DefaultPlayerPosition {
  IDLE_LEFT = 'defaultPlayerIdleLeft',
  IDLE_RIGHT = 'defaultPlayerIRight',
  IDLE_UP = 'defaultPlayerIUp',
  IDLE_DOWN = 'defaultPlayerIDown',

  WALK_LEFT = 'defaultPlayerWalkLeft',
  WALK_RIGHT = 'defaultPlayerWalkRight',
  WALK_UP = 'defaultPlayerWalkUp',
  WALK_DOWN = 'defaultPlayerWalkDown',

  ATTACK_LEFT = 'defaultPlayerAttackLeft',
  ATTACK_RIGHT = 'defaultPlayerAttackRight',
  ATTACK_UP = 'defaultPlayerAttackUp',
  ATTACK_DOWN = 'defaultPlayerAttackDown',
}

/**
 * Represents all enemy sprite positions.
 */
export enum SkeletonEnemyPositions {
  IDLE_LEFT = 'skeletonEnemyIdleLeft',
  IDLE_RIGHT = 'skeletonEnemyIdleRight',
  IDLE_UP = 'skeletonEnemyIdleUp',
  IDLE_DOWN = 'skeletonEnemyIdleDown',

  WALK_LEFT = 'skeletonEnemyWalkLeft',
  WALK_RIGHT = 'skeletonEnemyWalkRight',
  WALK_UP = 'skeletonEnemyWalkUp',
  WALK_DOWN = 'skeletonEnemyWalkDown',
}

/**
 * Represents all misc assets ids.
 */
export enum Misc {
  CHEST = 'chest',
  BUSH = 'bush',
  COIN = 'coin',
  GRASS = 'grass',
  HEART = 'heart',
  ROCK = 'rock',
  TREE_TRUNK = 'treeTrunk',
}
